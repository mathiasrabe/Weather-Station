...
;  Global Symbols
;-----------------------------------------------
export  PGA_sun_Start
export _PGA_sun_Start
export  PGA_sun_SetPower
export _PGA_sun_SetPower

export  PGA_sun_SetGain
export _PGA_sun_SetGain

export  PGA_sun_Stop
export _PGA_sun_Stop

IF (PGA_sun_AGNDBUFAPI)
export  PGA_sun_EnableAGNDBuffer
export _PGA_sun_EnableAGNDBuffer
export  PGA_sun_DisableAGNDBuffer
export _PGA_sun_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_sun_Start
;  FUNCTION NAME: PGA_sun_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[PGA_sun_GAIN_CR2]                           ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[PGA_sun_GAIN_CR2], A                      ; move complete value back to register
   pop  A
...
;
   ...
   mov  A, reg[PGA_sun_GAIN_CR0]                           ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[PGA_sun_GAIN_CR3],~EXGAIN                      ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_sun_GAIN_CR0], A                           ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_sun_GAIN_CR0], A                           ; move complete value back to register
   or   reg[PGA_sun_GAIN_CR3], EXGAIN                      ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_PGA_sun_Stop:
   ...
   and REG[PGA_sun_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_sun_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_sun_EnableAGNDBuffer:
_PGA_sun_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[PGA_sun_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_sun_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_sun_DisableAGNDBuffer:
_PGA_sun_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[PGA_sun_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File PGA_sun.asm
