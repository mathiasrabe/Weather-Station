...
;  Global Symbols
;-----------------------------------------------
export  PGA_wind_Start
export _PGA_wind_Start
export  PGA_wind_SetPower
export _PGA_wind_SetPower

export  PGA_wind_SetGain
export _PGA_wind_SetGain

export  PGA_wind_Stop
export _PGA_wind_Stop

IF (PGA_wind_AGNDBUFAPI)
export  PGA_wind_EnableAGNDBuffer
export _PGA_wind_EnableAGNDBuffer
export  PGA_wind_DisableAGNDBuffer
export _PGA_wind_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_wind_Start
;  FUNCTION NAME: PGA_wind_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[PGA_wind_GAIN_CR2]                          ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[PGA_wind_GAIN_CR2], A                     ; move complete value back to register
   pop  A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_wind_SetGain
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[PGA_wind_GAIN_CR0]                          ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[PGA_wind_GAIN_CR3],~EXGAIN                     ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_wind_GAIN_CR0], A                          ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_wind_GAIN_CR0], A                          ; move complete value back to register
   or   reg[PGA_wind_GAIN_CR3], EXGAIN                     ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_PGA_wind_Stop:
   ...
   and REG[PGA_wind_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_wind_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_wind_EnableAGNDBuffer:
_PGA_wind_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[PGA_wind_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_wind_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_wind_DisableAGNDBuffer:
_PGA_wind_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[PGA_wind_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File PGA_wind.asm
